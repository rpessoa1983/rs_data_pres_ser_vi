...
```{r echo=FALSE,eval=TRUE,tidy=FALSE,message=FALSE,warning=FALSE,prompt=FALSE,cache=FALSE,error=FALSE,collapse=TRUE}
...
dados_total<-dadocasamentos %>% filter(`Grupo de idade da mulher (Código)` %in% c(0)) %>%
  ...
  filter(Ano %in% c("2002")) %>% select(`Município (Código)`,Valor) %>% 
